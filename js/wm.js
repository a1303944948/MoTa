...
//ES6的监听对象变化方法
function objMonitor(object){
	//页面加载时初次渲染
	let domAll = dataBind();
	for(let i of domAll){
		let stringBind = String(i.dataset.bind).split('.');
		if(stringBind[0]&&stringBind[0] !== 'undefined'){
			let text;
			function stringBindText(bindArr,obj){
				text = obj[bindArr[1]];
				bindArr.splice(1,1);
				if(bindArr.length >= 2){
					stringBindText(bindArr,text);
				}else{
					i.innerHTML = text;
				}
			}
			stringBind.length >=2?stringBindText(stringBind,object[stringBind[0]]):i.innerHTML = object[stringBind[0]];
		}
	}
	//遍历所有的dom元素
	function dataBind(){
	...
	return new Proxy(object,{
		set: (e,txt,x)=>{
			...
			for(let i of domAll){
				let stringBind = String(i.dataset.bind).split('.');
				if(stringBind[0]&&stringBind[0] === txt){
					let text;
					function stringBindText(bindArr,obj){
						text = obj[bindArr[1]];
						bindArr.splice(1,1);
					...
					stringBind.length >=2?stringBindText(stringBind,x):i.innerHTML = x;
				}
			}
		},
	})
